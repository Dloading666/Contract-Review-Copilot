...
Dispatches to legacy or LangGraph orchestrator based on REVIEW_ORCHESTRATOR env var.
"""
...
async def _run_langgraph_review_stream(
    ...
    *,
    ...
    """LangGraph orchestrator entry point with checkpoint support."""
    ...
    # Section 2: Get checkpointer from lifecycle
    checkpointer = None
    ...
    # Section 1: Build config for checkpoint
    config = None
    ...
    # Section 1: Determine initial state vs resume
    if resume and checkpointer is not None:
        # Verify session ownership before resume
        ...
        try:
            ...
            if existing is None:
                print(f"[LangGraph] No checkpoint found for {session_id}, full rerun", flush=True)
                initial_state = {
                ...
                print(f"[LangGraph] Resuming from checkpoint for {session_id}", flush=True)
                initial_state = None  # Resume from checkpoint
        except Exception as exc:
            print(f"[LangGraph] Checkpoint lookup failed: {exc}, full rerun", flush=True)
            initial_state = {
                "session_id": session_id,
                "contract_text": contract_text,
                "model_key": model_key,
    ...
    # Section 1: Pass durability setting
...
# --- Legacy implementation (preserved verbatim from original) ---
    ...
    try:
        ...
        if model_task is not None and not model_task.done():
            try:
                ...
                print(f"[ReviewGraph] Deep model review failed: {exc}", flush=True)
        ...
        if not initial_ready:
            ...
            if fallback_issues:
                for issue in fallback_issues:
                    yield _sse_event("logic_review", {"session_id": session_id, "issue": issue})
...
# --- Helper functions for legacy ---

def _build_review_summary(issues: list[dict], *, deep_complete: bool) -> str:
    critical = sum(1 for issue in issues if _SEVERITY_ORDER.get(issue.get("level", "low"), 1) >= 4)
    high = sum(1 for issue in issues if _SEVERITY_ORDER.get(issue.get("level", "low"), 1) == 3)
    medium = sum(1 for issue in issues if _SEVERITY_ORDER.get(issue.get("level", "low"), 1) == 2)
    substantive = critical + high + medium
    if substantive == 0:
        return "合同分析已完成，当前未发现明显不公平条款。" if deep_complete else "阶段性审查未发现明显不公平条款，正在补全完整分析。"
    prefix = "合同分析已完成" if deep_complete else "阶段性审查已完成"
    return f"{prefix}，已识别 {substantive} 处风险：{critical} 处高危、{high} 处高风险、{medium} 处提示。"


def _build_finding_changes(initial_issues: list[dict], deep_issues: list[dict]) -> list[dict]:
    def _issue_key(issue: dict) -> str:
        return f"{str(issue.get('clause', '')).strip().lower()}|{str(issue.get('issue', '')).strip().lower()}"
    def _severity_rank(issue: dict) -> int:
        return _SEVERITY_ORDER.get(str(issue.get("level", "low")).lower(), 1)
    ...
    for issue in deep_issues:
        ...
        elif _severity_rank(issue) > _severity_rank(prior):
            changes.append({**issue, "change_type": "upgraded"})
    return changes


async def _await_with_heartbeat(task, *, session_id, stage, message, heartbeat_seconds):
    while True:
        try:
            ...
            yield _sse_event("deep_review_heartbeat", {
                "session_id": session_id, "stage": stage,
                "message": message, "completed": False,
            })
...
async def run_aggregation_stream(contract_text, session_id, issues=None, model_key=None):
    ...
    for index, paragraph in enumerate(paragraphs):
        yield _sse_event("final_report", {
            "session_id": session_id, "paragraph": paragraph,
            "is_last": index == len(paragraphs) - 1,
        })
    yield _sse_event("review_complete", {"session_id": session_id})
...
async def run_deep_review_stream(contract_text, session_id, issues=None, model_key=None):
    ...
    yield _sse_event("deep_review_started", {
        "session_id": session_id, "review_stage": "deep",
    ...
    try:
        ...
        if pgvector_results and routing.get("primary_source") == "pgvector":
            yield _sse_event("rag_retrieval", {
                "session_id": session_id, "source": "pgvector",
                "documents": [{"title": c.get("metadata", {}).get("title", "法律条款"), "content": c.get("chunk_text", ""), "score": float(c.get("similarity", 0))} for c in pgvector_results],
            })
        model_task = asyncio.create_task(_run_sync(model_review_clauses, contract_text, routing, extracted_entities, model_key, timeout=settings.review_model_timeout_seconds, allow_retry=True))
        deep_issues = initial_issues
        model_review_degraded = False
        try:
            async for heartbeat_event in _await_with_heartbeat(model_task, session_id=session_id, stage="model_review", message="合同分析仍在进行中...", heartbeat_seconds=settings.review_heartbeat_interval_seconds):
                if heartbeat_event["event"] == "_task_result":
                    model_issues = heartbeat_event["data"]["result"]
                    from ..agents.logic_review import _merge_issue_lists
            ...
            model_review_degraded = True
            print(f"[ReviewGraph] Deep model review resume failed: {exc}", flush=True)
        finding_changes = _build_finding_changes(initial_issues, deep_issues)
        if finding_changes or model_review_degraded:
            yield _sse_event("deep_review_update", {
                "session_id": session_id, "review_stage": "deep",
                "summary": _build_review_summary(deep_issues, deep_complete=False),
                "message": f"合同分析补充了 {len(finding_changes)} 处新的或升级后的分析结果。" if finding_changes else "模型暂时超时，已基于当前扫描结果继续生成完整报告。",
                "issues": deep_issues, "changes": finding_changes, "degraded": model_review_degraded,
            })
        report_task = asyncio.create_task(_run_sync(generate_report, contract_text, deep_issues, model_key))
        paragraphs: list[str] = []
        async for heartbeat_event in _await_with_heartbeat(report_task, session_id=session_id, stage="report_generation", message="完整报告仍在生成中...", heartbeat_seconds=settings.review_heartbeat_interval_seconds):
            if heartbeat_event["event"] == "_task_result":
                paragraphs = heartbeat_event["data"]["result"]
                break
            yield heartbeat_event
        for index, paragraph in enumerate(paragraphs):
            yield _sse_event("final_report", {"session_id": session_id, "paragraph": paragraph, "is_last": index == len(paragraphs) - 1})
        yield _sse_event("deep_review_complete", {
            "session_id": session_id, "review_stage": "deep",
            "summary": _build_review_summary(deep_issues, deep_complete=True),
            "message": "合同分析已完成，页面内容已自动更新。",
            "issues": deep_issues, "changes": finding_changes,
        })
        yield _sse_event("review_complete", {"session_id": session_id})
    except Exception as exc:
        print(f"[ReviewGraph] Deep review resume failed: {exc}", flush=True)
        yield _sse_event("deep_review_failed", {"session_id": session_id, "message": "完整分析暂未补全，你可以再次补全分析。"})
        yield _sse_event("review_complete", {"session_id": session_id, "degraded": True})

...
def test_create_chat_completion_returns_cached_response(monkeypatch):
    monkeypatch.setattr(
        entity_extraction,
        'get_json',
        lambda key: {'content': 'cached-response', 'model': 'glm-5'},
    )
    monkeypatch.setattr(
        entity_extraction,
        'get_llm_client',
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('LLM should not be called on cache hit')),
    )
...
def test_search_web_uses_cached_results(monkeypatch):
    ...
    monkeypatch.setattr(duckduckgo, 'get_json', lambda key: cached_results)
...
def test_load_paused_session_reads_from_cache(monkeypatch):
    ...
    monkeypatch.setattr(main, 'get_json', lambda key: expected)
...
def test_store_paused_session_keeps_memory_copy_when_cache_write_fails(monkeypatch):
    ...
    main.paused_sessions.clear()
    monkeypatch.setattr(main, 'get_ttl_seconds', lambda kind: 7200)
    monkeypatch.setattr(main, 'set_json', lambda key, value, ttl: capture.update({'key': key, 'value': value, 'ttl': ttl}) or False)
